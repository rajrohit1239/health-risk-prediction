...
if st.session_state.page == 'input':
    ...
    with col1:
        ...
        bmi = st.number_input("⚖️ BMI Index", 10.0, 40.0, 22.0)
    ...
    if st.button("🔍 ANALYZE PATIENT PROFILE"):
        ...
        with placeholder.container():
            st.markdown("""
                <div class="loading-overlay">
                    ...
                    <h2 style="margin-top:20px;">Scanning Health Patterns...</h2>
                </div>
            """, unsafe_allow_html=True)
            time.sleep(4.0) 
        ...
        if model and encoders:
            ...
            st.session_state.prediction_results = {
                'label': label,
                'metrics': {"Age": age, "Exercise": exercise, "Sleep": sleep, "Stress": encoders['stress'].transform([stress])[0]}
            }
    ...
    if "Low" in res['label']:
        color = "#4ade80"
        suggestion = "Great job! Maintain your current lifestyle, stay active, and keep a balanced diet."
    elif "Medium" in res['label']:
        color = "#fbbf24"
        suggestion = "Take care! Consider increasing physical activity and managing stress levels for better health."
    else:
        color = "#f87171"
        suggestion = "Action required! Please consult a healthcare professional and focus on improving your diet and exercise habits."
    ...
    with c2:
        ...
        fig = px.bar(df, x='Factor', y='Value', color='Value', color_continuous_scale='Purples')
